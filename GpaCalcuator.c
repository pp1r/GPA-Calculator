...
{
 FILE *fp;
 fp = fopen("Record", "a");  // Opening the file in append mode
 printf("Enter the Roll no   :");
 scanf("%d", &stud.rollno);  // Reading roll number from user
 printf("Enter the Name      :");
 scanf("%s", stud.name);    // Reading name from user
 printf("Enter the mark      :");
 scanf("%f", &stud.mark);    // Reading mark from user
 fwrite(&stud, sizeof(stud), 1, fp);  // Writing the student information to the file
...
{
 FILE *fp1;
 fp1 = fopen("Record", "r");  // Opening the file in read mode
 printf("\nRoll Number\tName\tMark\n\n");
 while (fread(&stud, sizeof(stud), 1, fp1))
 // Loop to read all the records from the file and print them
...
void search()
{
 ...
 scanf("%d", &r);
 avl = avlrollno(r);  // Checking if the roll number is available in the file
 if (avl == 0)
  printf("Roll No %d is not available in the file\n",r);
 else
 {
  fp2 = fopen("Record", "r");  // Opening the file in read mode
  while (fread(&stud, sizeof(stud), 1, fp2))
  {
   s = stud.rollno;
   if (s == r)
   {
    // If the roll number is found, printing the student information
